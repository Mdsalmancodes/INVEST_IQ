"""
INVEST IQ - REAL XGBOOST INFERENCE E2E TEST
...
Pipeline:
    ...
    17 technical features
            ↓
    Active XGBoost ModelVersion
            ↓
    ModelLoader
            ↓
    XGBoost .pkl artifact
            ↓
...
Important:
    ...
    • Verifies persisted feature schema
    • Verifies exact feature ordering
...
def build_ohlcv_dataframe(
    ...
    """
    Convert MarketDataRepository bars into the canonical OHLCV DataFrame.
    """
    ...
    if dataframe.empty:
        raise RuntimeError(
            "All OHLCV timestamps are invalid."
        )
...
def validate_ohlcv(
    ...
    if not dataframe.index.is_monotonic_increasing:
        raise RuntimeError(
            "OHLCV dataframe is not chronologically sorted."
        )
...
def prepare_latest_features(
    ...
    if raw_features.empty:
        raise RuntimeError(
            "FeatureEngineer returned an empty feature dataframe."
        )
    ...
    if not np.isfinite(
        ...
        raise RuntimeError(
            "Valid feature dataframe contains "
    ...
    if latest_features.shape[0] != 1:
        raise RuntimeError(
            "Latest feature dataframe must contain exactly one row."
        )
...
def validate_feature_schema(
    ...
    """
    Verify exact feature schema compatibility.
    """
    ...
    if not trained:
        raise RuntimeError(
            "XGBoost artifact contains no persisted "
            "feature schema."
        )
...
async def main() -> None:
    ...
    print()
    print_separator()
    print("3. ACTIVE XGBOOST MODEL LOOKUP")
    print_separator()
    ...
    if active_model is None:
        raise RuntimeError(
            f"No active XGBoost model found for {SYMBOL}."
        )

    print(
        "✅ Active XGBoost model found."
    )
    ...
    if str(
        ...
        raise RuntimeError(
            "Active XGBoost model is not active."
        )
    ...
    if not artifact_path.exists():
        raise RuntimeError(
            "Registered XGBoost artifact does not exist:\n"
            f"{artifact_path}"
        )

    if not artifact_path.is_file():
        raise RuntimeError(
            "Registered XGBoost artifact is not a file."
        )

    if artifact_path.suffix.lower() != ".pkl":
        raise RuntimeError(
            "XGBoost artifact must use .pkl extension."
        )
    ...
    if raw_features.shape[1] != EXPECTED_FEATURE_COUNT:
        raise RuntimeError(
            "Unexpected technical feature count.\n"
    ...
    # ========================================================================
    # 8. INFERENCE FEATURE PREPARATION
    # ========================================================================
    ...
    if latest_features.shape != (
        ...
        raise RuntimeError(
            "Latest feature row has incorrect shape.\n"
            f"Expected: (1, {EXPECTED_FEATURE_COUNT})\n"
            f"Actual: {latest_features.shape}"
        )
    ...
    # ========================================================================
    # 10. LOAD ACTIVE XGBOOST
    # ========================================================================

    print()
    print_separator()
    print("10. LOAD ACTIVE XGBOOST")
    print_separator()

    print(
        "📦 Loading active XGBoost artifact..."
    )
    ...
    print()
    print_separator()
    print("11. XGBOOST MODEL VALIDATION")
    print_separator()
    ...
    if not is_fitted:
        raise RuntimeError(
            "Loaded XGBoost model is not fitted."
        )
    ...
    if len(trained_feature_names) != EXPECTED_FEATURE_COUNT:
        raise RuntimeError(
            "Persisted XGBoost schema does not contain "
    ...
    print()
    print(
        "✅ XGBoost training schema contains 17 features."
    )

    # ========================================================================
    # 13. XGBOOST INFERENCE
    # ========================================================================

    print()
    print_separator()
    print("13. XGBOOST PREDICTION")
    print_separator()
    ...
    buy_probability_array = np.asarray(
        buy_probability_array,
        dtype=np.float64,
    ).reshape(-1)

    sell_probability_array = np.asarray(
        sell_probability_array,
        dtype=np.float64,
    ).reshape(-1)

    if buy_probability_array.size != 1:
        raise RuntimeError(
            "Expected exactly one BUY probability."
        )

    if sell_probability_array.size != 1:
        raise RuntimeError(
            "Expected exactly one SELL probability."
        )

    buy_probability = float(
        buy_probability_array[0]
    )

    sell_probability = float(
        sell_probability_array[0]
    )
    ...
    if not np.isfinite(
        ...
        raise RuntimeError(
            "SELL probability is not finite."
        )
    ...
    if (
        buy_probability
        > sell_probability + SIGNAL_EPSILON
    ):
        signal = "UP"

    elif (
        sell_probability
        > buy_probability + SIGNAL_EPSILON
    ):
        signal = "DOWN"
    ...
    print(
        f"Probability UP     : "
        f"{buy_probability:.6f}"
    )

    print(
        f"Probability DOWN   : "
        f"{sell_probability:.6f}"
    )
    ...
    if not (
        0.0 <= buy_probability <= 1.0
    ):
        raise RuntimeError(
            f"BUY probability outside [0, 1]: "
            f"{buy_probability}"
        )

    print(
        "✅ BUY probability is valid."
    )

    if not (
        0.0 <= sell_probability <= 1.0
    ):
        raise RuntimeError(
            f"SELL probability outside [0, 1]: "
            f"{sell_probability}"
        )

    print(
        "✅ SELL probability is valid."
    )
    ...
    if not np.isclose(
        ...
        raise RuntimeError(
            "BUY + SELL probabilities do not sum to 1.\n"
            f"BUY: {buy_probability}\n"
            f"SELL: {sell_probability}\n"
            f"SUM: {probability_sum}"
        )

    print(
        "✅ BUY + SELL probabilities sum to 1."
    )
    ...
    if registered_artifact != artifact_path:
        raise RuntimeError(
    ...
    print()
    print_separator()
    print("18. FINAL XGBOOST INFERENCE PIPELINE")
    print_separator()
    ...
    print("        ↓")
    print("ACTIVE XGBOOST MODEL REGISTRY")
    print("        ↓")
    print("MODEL LOADER")
    print("        ↓")
    print("XGBOOST .PKL ARTIFACT")
    print("        ↓")
    ...
    print_separator()
    print(
        "🎉 XGBOOST REAL INFERENCE E2E TEST PASSED"
    )
    print_separator()
    ...
    print(
        "✅ REAL XGBOOST PREDICTION GENERATED"
    )

    print(
        "✅ BUY/SELL PROBABILITIES VERIFIED"
    )
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_xgboost_real_inference_end_to_end() -> None:
    """
    Pytest entry point for the real XGBoost inference E2E test.

    This executes the same production-style pipeline used by main():
