"""
INVEST IQ - REAL RANDOM FOREST TRAINING TEST
...
@pytest.mark.asyncio
async def test_random_forest_real_training_end_to_end() -> None:
    """
    Train Random Forest using real AAPL market data and verify:

    1. Market data can be downloaded.
    2. Training use case executes successfully.
    3. ModelVersion is created.
    4. Validation metrics are generated.
    5. Random Forest .pkl artifact exists.
    6. Artifact is non-empty.
    7. ModelVersion is registered.
    8. Registered metadata matches the trained model.
    9. An active AAPL Random Forest model exists.
    10. The newly trained version is the active version.
    """

    print()
    print("=" * 78)
    print("INVEST IQ - REAL RANDOM FOREST TRAINING TEST")
    print("=" * 78)
    ...
    print()
    print("=" * 78)
    print("5. STARTING REAL RANDOM FOREST TRAINING")
    print("=" * 78)

    print()
    print(
        "Real market data will be downloaded from Yahoo Finance."
    )

    print(
        "Random Forest will be trained using the engineered "
        "technical features."
    )
    ...
    assert model_version.family == MODEL_FAMILY, (
        "ModelVersion family does not match training family."
    )

    print("Model family is correct.")

    assert model_version.symbol == SYMBOL, (
        "ModelVersion symbol does not match training symbol."
    )
    ...
    assert artifact_size > 0, (
        "Model artifact exists but is empty."
    )
    ...
    assert artifact_path.suffix.lower() == ".pkl", (
        "Random Forest artifact must have .pkl extension."
    )

    print("Artifact extension is .pkl.")

    # ========================================================================
    # 11. MODEL REGISTRY VERIFICATION
    # ========================================================================
    ...
    assert registered is not None, (
        "ModelVersion was not found in the filesystem registry."
    )
    ...
    # ========================================================================
    # 12. REGISTERED MODEL CONSISTENCY
    # ========================================================================
    ...
    assert registered.family == MODEL_FAMILY, (
        "Registered model family does not match."
    )

    print("Family matches.")

    assert registered.symbol == SYMBOL, (
        "Registered model symbol does not match."
    )
    ...
    assert family_versions, (
        "No model versions found for the family."
    )

    # ========================================================================
    # 14. FIND ACTIVE AAPL MODEL
    # ========================================================================
    ...
    assert active_model.family == MODEL_FAMILY, (
        "Active model family does not match Random Forest."
    )

    print("Active family matches Random Forest.")
    ...
    # ========================================================================
    # 16. FINAL PIPELINE VERIFICATION
    # ========================================================================

    print()
    print("=" * 78)
    print("16. FINAL PIPELINE VERIFICATION")
    print("=" * 78)
    ...
    print()
    print("=" * 78)
    print("RANDOM FOREST END-TO-END TEST PASSED")
    print("=" * 78)
    ...
    print("AAPL ACTIVE MODEL FOUND")
    print("REGISTRY CONSISTENCY VERIFIED")

    print()
    print(
        "INVEST IQ RANDOM FOREST MODEL "
